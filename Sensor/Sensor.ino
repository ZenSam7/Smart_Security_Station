...
int offsets[9];   /* Помещяем сюда первоначальные значения датчиков по осям x, y, z
(сначала акселерометра, потом гироскопа, потом магнитометра)
...
const uint8_t frequency_data_of_gyro = 40; /* Как часто будем снимать показания с гироскопа, для определения движения коробки
(Сколько раз в секунду)  (самое большое: 40)*/

const uint8_t number_entries = 2 * 3 * frequency_data_of_gyro; /* Количество записей в массив moving_of_box
  2 = количество секунд записи, 3 = количество осей*/

int moving_of_box[number_entries]; // Куда записываем данные с коробки

float sensitive_gyr = 131.072 *0.8; // Чувствительность гироскопа       (В данном случае единица значения == 1°/s )
float sensitive_acc = 40.96;        // Чувствительность акселерометра   ¡В данном случае единица значения == 0.01g!
float sensitive_mag = 2.38;         // Чувствительность магнитометра
...
void check_for_rotation(bool mode) { // При получении сообщения со станции
  ...
  */
  ...
  bool apparatus_tilted = 0;    // Аппарат наклонён?
    ...
    if (millis() - timer_frequency >= 1000 / frequency_data_of_gyro) {
      ...
      for (int axes_sum_index=0; axes_sum_index < 3; axes_sum_index++) {   // По каждой оси
        ...
        for (uint16_t sum_index = axes_sum_index;
          ...
          // Для повышения точности мы записывали и 2 знака после запятой ( *100 )
          double a = ((double) ((double) moving_of_box[sum_index] / sensitive_gyr) / frequency_data_of_gyro) *100;
          summ += abs(a);
        }

        // И проверяем, есть ли поворот на >80°
        if (summ >= 80*100) { 
          apparatus_tilted = true; // Аппарат повернулся достаточно!
        }
        Serial.print(summ/100.0);Serial.print('\t');
      } Serial.print("\n");
    }
  }
...
int* filter(int* list[], int mean_range) {
  int* new_list = new int[number_entries];

  // Убираем краюшки
  for (int i=0; i<mean_range; i++) {new_list[i] = list[i];}
  for (int i=number_entries-mean_range-1; i<number_entries; i++) {new_list[i] = list[i];}
  
  for (int i=mean_range; i<number_entries-mean_range; i++) {
    int wind[2*mean_range +1];
    for (int q=i; q<i+2*mean_range +1; q++){wind[q] = list[q];}

    // Сортируем
    int* temp = bubble_sort(wind);
    for (int q=0; q<2*mean_range +1; q++){
      wind[q] = temp[q];
    }
  }
